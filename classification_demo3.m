...
fid = fopen([datapath 'CRABS.DAT']);
names = textscan(fid,'%s %s %s %s %s %s %s %s',1);
data = textscan(fid,'%c %c %d %f %f %f %f %f');
fclose(fid);
X = [data{4} data{5} data{6} data{7} data{8}];
% X = zscore(X);
...
perm = randperm(50);
index1 = [perm(1:20) 50+perm(1:20) 100+perm(1:20) 150+perm(1:20)];
