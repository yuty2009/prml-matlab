...
char *kernel_name[] = {"linear","poly","rbf","sigmoid","gpkernel","custom"};
...
double kernel( long a, long b )
{
   ...
   switch( ker ) {
      ...
      /* gpkernel */
      case 4:
         c = arg1[0]*exp( -0.5*arg1[1]*sub_dot_prod( a, b) ) + arg1[2] + arg1[3]*dot_prod( a, b );
         break;
...
/* --------------------------------------------------------------------
 Computes derivation of kernel function wrt. kernel args for a-th and b-th.

 The base address for the 1st argument is dataA and for the 2nd
 argument is dataB.
-------------------------------------------------------------------- */
double kderiv( long a, long b, long k )
{
    double c = 0;
    ...
    switch( ker ) {
        ...
        case 1:
            if( k == 0 ) {
                c = log( dot_prod( a, b) + arg1[1] ) * pow( (dot_prod( a, b) + arg1[1]), arg1[0] );
            } else if( k == 1 ) {
                c = arg1[0] * pow( (dot_prod( a, b) + arg1[1]), arg1[0]-1 );
            }
            break;
            /* radial basis functions kernel*/
        case 2:
            if( k == 0 ) {
                c = pow(arg1[0],-3) * sub_dot_prod( a, b) * exp( -0.5*sub_dot_prod( a, b)/(arg1[0]*arg1[0]) );
            }
            break;
            /* sigmoid */
        case 3:
            if( k == 0 ) {
                c = (1 - pow( tanh( arg1[0]*dot_prod( a,b) + arg1[1] ), 2 )) * dot_prod( a,b);
            } else if( k == 1 ) {
                c = 1 - pow( tanh( arg1[0]*dot_prod( a,b) + arg1[1] ), 2 );
            }
            break;
            /* gpkernel */
        case 4:
            if( k == 0 ) {
                c = exp( -0.5*arg1[1]*sub_dot_prod( a, b) );
            } else if( k == 1 ) {
                c = -0.5*arg1[0]*sub_dot_prod( a, b)*exp( -0.5*arg1[1]*sub_dot_prod( a, b) );
            } else if( k == 2 ) {
                c = 1;
            } else if( k == 3 ) {
                c = dot_prod( a, b);
