%% Bayes linear regression with known correlations
% X: N by P feature matrix, N number of samples, P number of features
% y: N by 1 target vector
% K: P by P matrix of known correlation between variables 
%    (e.g., covariance matrix)
% b: P by 1 regression coefficients
% b0: the intercept
function [varargout] = bardpriorreg(y, X, K)
...
while (d_w > stopeps)  && (i < maxit)
    ...
    gamma1 = 1 - alphas1.*(diagSigma1+diagD1);
    alphas1 = max(gamma1,eps)./(w1.^2+1e-32) - rho;
    % alphas1 = (1-alphas1.*diagSigma1-rho./(alphas1+rho))./(w1.^2+1e-32);
    alphas(index1) = max(alphas1,eps); % ensure alpha is positive
    % rho = rho*trace((diag(1./alphas1)-w1*w1'-Sigma1)*K)/(trace(D*K)+1e-32);
